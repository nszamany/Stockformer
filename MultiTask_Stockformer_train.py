...
import gc
import psutil, os  # optional: for memory monitoring (remove if not desired)
...
# Extract date range from config filename
import re  # make sure to add this at the top if not already imported
config_filename = os.path.basename(args.config)
m = re.search(r'(\d{4}-\d{2}-\d{2}_\d{4}-\d{2}-\d{2})', config_filename)
date_range = m.group(1) if m else "unknown_date"
...
tensorboard_folder = rf"C:\Users\ns243\Documents\Academic\AI Master\Internship\Data\output\runs\Multitask_Stockformer\SP500_{date_range}"
...
def test_res(model, valXL, valXH, valXC, bonus_valX, valTE, valY, valYC, adjgat):
    ...
    base_output_dir = rf"C:\Users\ns243\Documents\Academic\AI Master\Internship\Data\output\Multitask_output_{date_range}"
    os.makedirs(os.path.join(base_output_dir, "classification"), exist_ok=True)
    os.makedirs(os.path.join(base_output_dir, "regression"), exist_ok=True)
    save_to_csv(os.path.join(base_output_dir, "classification", "classification_pred_last_step.csv"), pred_class[:, -1, :])
    save_to_csv(os.path.join(base_output_dir, "classification", "classification_label_last_step.csv"), label_class[:, -1])
    save_to_csv(os.path.join(base_output_dir, "regression", "regression_pred_last_step.csv"), pred_regress[:, -1, :])
    save_to_csv(os.path.join(base_output_dir, "regression", "regression_label_last_step.csv"), label_regress[:, -1])
...
def train(model, trainXL, trainXH, trainXC, bonus_trainX, trainTE, trainY, trainYL, trainYC,
          valXL, valXH, valXC, bonus_valX, valTE, valY, valYC, adjgat):
    import gc, psutil, os
    ...
    for epoch in tqdm(range(1, args.max_epoch + 1)):
        ...
        with tqdm(total=num_batch) as pbar:
            for batch_idx in range(num_batch):
                ...
                # clean batch memory
                del xl, xh, xc, y, yl, yc, te, bonus, loss, loss_regress, loss_class
                torch.cuda.empty_cache()
                gc.collect()

        log_string(
            log,
            'epoch %d, lr %.6f, loss %.4f, time %.1f sec'
        ...
        log_string(log, f"Epoch {epoch} done, memory usage: {mem_gb:.2f} GB")
